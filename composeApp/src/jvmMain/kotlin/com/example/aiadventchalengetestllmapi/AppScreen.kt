...
private fun appReadApiKey(envVar: String): String {
...
private fun Double.appFormatSeconds(): String = String.format(Locale.US, "%.2f", this)

@OptIn(ExperimentalMaterial3Api::class)
@Composable
@Preview
fun App(onBackClick: () -> Unit = {}) {
    MaterialTheme {
        ...
        val apiKeysByApi = remember {
            mutableStateMapOf<AppApi, String>().apply {
                AppApi.entries.forEach { api ->
                    this[api] = appReadApiKey(api.envVar)
                }
            }
        }
        var selectedApi by remember { mutableStateOf(AppApi.DeepSeek) }
        ...
        var modelInput by remember { mutableStateOf(AppApi.DeepSeek.defaultModel) }
...
@Composable
private fun AppBubble(message: AppMessage) {
