...
actual class AiStateAgentDatabaseDriverFactory {
    actual fun createDriver(): SqlDriver {
        ...
        if (!hasCompatibleSchema(dbPath)) {
            runCatching { AiStateAgentDatabase.Schema.create(driver) }
        }
        if (!hasCompatibleSchema(dbPath)) {
            driver.close()
            deleteDatabaseFiles(dbPath)
            driver = openDriver(dbPath)
            AiStateAgentDatabase.Schema.create(driver)
        }
        return driver
    }
    ...
    private fun defaultDatabasePath(): Path {
        ...
        return Paths.get(userHome, ".aiadventchalengetestllmapi", "ai_state_agent.db").also {
