...
actual class AiAgentRagDatabaseDriverFactory {
    actual fun createDriver(): SqlDriver {
        ...
        if (!hasCompatibleSchema(dbPath)) {
            runCatching { AiAgentRagDatabase.Schema.create(driver) }
        }
        if (!hasCompatibleSchema(dbPath)) {
            driver.close()
            deleteDatabaseFiles(dbPath)
            driver = openDriver(dbPath)
            AiAgentRagDatabase.Schema.create(driver)
        }
        return driver
    }
    ...
    private fun defaultDatabasePath(): Path {
        ...
        return Paths.get(userHome, ".aiadventchalengetestllmapi", "ai_agent_rag.db").also {
